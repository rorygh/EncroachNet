...
2. **Camera Pose Resolution**: two paths — reuse existing LiDAR-flight direct georeferencing (PPK/RTK+IMU) when a co-registered LiDAR channel is present, or run COLMAP SfM/MVS (reusing the pipeline at `C:\rory\scripts\LogMotion`) for RGB-only flights.
...
- **No combined dataset exists**: powerline datasets (TTPLA, InsPLAD) don't label vegetation; vegetation datasets (UAVid, Semantic Drone, VDD) don't label power lines. Requires merged-taxonomy pretraining or synthetic data — see [docs/datasets.md](docs/datasets.md).
...
- **Local reusable tooling**: `C:\rory\scripts\LogMotion` (working COLMAP SfM/MVS pipeline), `C:\rory\scripts\search3d\...\semantic_sam` (vendored Mask2Former/Semantic-SAM codebase), Softgrove/SimpleUNet's spconv sparse U-Net pattern for the optional 3D refinement stage.
